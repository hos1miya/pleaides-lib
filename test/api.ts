...
describe('API', () => {
	test('success', async () => {
		...
		fetchMock.mockResponse(async (req) => {
			...
			if (req.method == 'POST' && req.url == 'https://pleaides.example/api/i') {
				if (body.i === 'TOKEN') {
		...
		expect(getFetchCall(fetchMock.mock.calls[0])).toEqual({
			url: 'https://pleaides.example/api/i',
			method: 'POST',
			body: { i: 'TOKEN' }
		});
	});
	...
	test('with params', async () => {
		...
		fetchMock.mockResponse(async (req) => {
			const body = await req.json();
			if (req.method == 'POST' && req.url == 'https://pleaides.example/api/notes/show') {
		...
		expect(getFetchCall(fetchMock.mock.calls[0])).toEqual({
			url: 'https://pleaides.example/api/notes/show',
			method: 'POST',
	...
	test('204 No Content で null が返る', async () => {
		fetchMock.resetMocks();
		fetchMock.mockResponse(async (req) => {
			if (req.method == 'POST' && req.url == 'https://pleaides.example/api/reset-password') {
				return { status: 204 };
			} else {
				return { status: 404 };
			}
		...
		expect(getFetchCall(fetchMock.mock.calls[0])).toEqual({
			url: 'https://pleaides.example/api/reset-password',
			method: 'POST',
